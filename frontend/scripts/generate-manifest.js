 ...
 * Versión optimizada para Vercel
 */
...
function loadTemplate() {
  // Rutas relativas desde frontend/scripts/
...
function verifyLogo(clientId) {
  const config = CLIENT_CONFIGS[clientId];
  const logoPath = path.join(__dirname, '../src', config.logoPath);
...
function main() {
  ...
  // 2. Verificar logo (no bloqueante)
  verifyLogo(clientId);
