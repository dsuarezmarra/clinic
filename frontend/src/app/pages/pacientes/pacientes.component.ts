...
export class PacientesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Configurar b�squeda con debounce para evitar race conditions
    this.searchSubscription = this.searchSubject.pipe(
      debounceTime(300), // Esperar 300ms despu�s de la �ltima tecla
      distinctUntilChanged(), // Solo si el valor cambi�
      tap(() => {
        this.loading = true;
        this.currentPage = 1; // Reiniciar a p�gina 1
      }),
  ...
  ngOnDestroy() {
    // Limpiar subscripci�n para evitar memory leaks
