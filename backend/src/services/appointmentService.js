...
/**
 * Servicio centralizado para gestiÛn de citas y consumo de crÈditos
 * Principios de diseÒo:
 * - Estado calculado: El estado "pagado" se calcula din·micamente basado en credit_packs.paid
 * - Transacciones atÛmicas: Las operaciones crÌticas usan transacciones para mantener consistencia
 * - Idempotencia: Las operaciones pueden ejecutarse m˙ltiples veces sin efectos secundarios
 * - SeparaciÛn de responsabilidades: Cada mÈtodo tiene una responsabilidad clara
...
class AppointmentService {
  ...
  /**
   * Crea una nueva instancia del servicio con un cliente de BD especÌfico
  ...
  /**
   * Calcula las unidades necesarias basado en la duraci√≥n
   */
  ...
  /**
   * Determina si una cita est√° pagada bas√°ndose en sus redemptions
   */
  ...
  /**
   * Obtiene los cr√©ditos disponibles de un paciente
   */
  ...
  /**
   * Consume crÈditos para una cita de forma idempotente
   */
  async consumeCredits(patientId, appointmentId, durationMinutes, transaction = null) {
    ...
    // Consumir cr√©ditos de forma optimizada
    ...
    if (remainingUnits > 0) {
      throw new Error(`Cr√©ditos insuficientes. Necesarios: ${requiredUnits}, consumidos: ${requiredUnits - remainingUnits}`);
    }
  }

  /**
   * Calcula cu√°ntas unidades usar de un pack espec√≠fico
   */
  _calculateUnitsToUse(pack, remainingUnits, totalRequired) {
    ...
    if (totalRequired === 2) {
      ...
      const isSesion60 = pack.label?.startsWith('Sesi√≥n') && pack.unitMinutes === 60;
      const isBono60 = pack.unitMinutes === 60 && !pack.label?.startsWith('Sesi√≥n');
  ...
  /**
   * Revierte el consumo de crÈditos
   */
  ...
  /**
   * Crea una nueva cita con validaciones y consumo de cr√©ditos
   */
  async createAppointment(data) {
    ...
    // Auto-detectar duraciÛn de 60min si hay packs disponibles
    ...
    if (consumesCredit && patientId) {
      ...
      if (availableUnits < requiredUnits) {
        const error = new Error(`Cr√©ditos insuficientes. Necesarios: ${requiredUnits}, disponibles: ${availableUnits}`);
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      // Consumir cr√©ditos si es necesario
      if (consumesCredit && patientId) {
  ...
  /**
   * Actualiza una cita existente con manejo inteligente de cr√©ditos
   */
  async updateAppointment(id, updates) {
    ...
    console.log(`[UPDATE APPOINTMENT] ${id} - An√°lisis:`, {
      currentlyPaid,
    ...
    return await this.prisma.$transaction(async (tx) => {
      // 1. Revertir crÈditos si es necesario (solo para cambios estructurales en citas no pagadas)
      if (hasStructuralChanges && currentAppointment.consumesCredit && !currentlyPaid) {
        console.log(`[UPDATE APPOINTMENT] Revirtiendo cr√©ditos para cambios estructurales`);
        await this.revertCredits(id, tx);
      }

      // 2. Actualizar datos b√°sicos de la cita
      ...
      // 4. Re-consumir cr√©ditos si hubo cambios estructurales y la cita no est√° pagada
      ...
      if (hasStructuralChanges && appointment.consumesCredit && !finalPaidStatus && paid !== true) {
        console.log(`[UPDATE APPOINTMENT] Re-consumiendo cr√©ditos para cambios estructurales`);
  ...
  /**
   * Determina si hay cambios estructurales que afectan el consumo de cr√©ditos
   */
  ...
  async _processPaymentStatusChange(appointment, requestedPaidStatus, currentlyPaid, tx) {
    ...
    if (requestedPaidStatus === true && !currentlyPaid) {
      ...
      console.log(`[UPDATE APPOINTMENT] Cita ${appointment.id} ya est√° pagada`);
    } else if (requestedPaidStatus === false && !currentlyPaid) {
      console.log(`[UPDATE APPOINTMENT] Cita ${appointment.id} ya est√° como no pagada`);
    }
  }
  ...
  /**
   * Cancela una cita y revierte sus cr√©ditos
   */
  async cancelAppointment(id) {
    ...
    return await this.prisma.$transaction(async (tx) => {
      // Revertir crÈditos si es necesario
