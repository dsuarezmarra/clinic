...
router.post('/restore/:fileName', async (req, res, next) => {
    ...
    console.log('馃攣 restore request:', { fileName, allowEnv, NODE_ENV: process.env.NODE_ENV, force, allowAutoRestore });
...
// GET /api/backup/cron - Endpoint para Vercel Cron Jobs (backup autom醫ico semanal)
// Este endpoint es llamado autom醫icamente por Vercel Cron seg鷑 la configuraci髇 en vercel.json
router.get('/cron', async (req, res, next) => {
    try {
        ...
        // Vercel env韆 el header Authorization con el valor Bearer <CRON_SECRET>
        // Si no hay CRON_SECRET configurado, permitimos la ejecuci髇 (para desarrollo)
        ...
        console.log('[CRON] Ejecutando backup semanal autom醫ico...');
        ...
        res.json({
            success: true,
            message: 'Backup semanal autom醫ico completado',
            type: 'weekly',
            timestamp: new Date().toISOString(),
            ...result
        });
        ...
        res.status(500).json({
            success: false,
            message: error.message || 'Error al crear backup autom醫ico'
        });
    }
});
