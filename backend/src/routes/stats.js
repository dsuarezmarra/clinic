...
function calculateAppointmentPrice(appointment) {
  ...
  // 3. Fallback: precio seg�n duraci�n
  const mins = Number(appointment.durationMinutes || 0);
  return mins >= 60 ? DEFAULT_PRICE_60 : DEFAULT_PRICE_30;
}

/**
 * Determina si una cita est� pagada bas�ndose en:
 * 1. Si tiene creditRedemptions y TODOS los packs est�n pagados
 * 2. Si tiene priceCents sin redemptions (sesi�n suelta pagada directamente)
 */
function isAppointmentPaid(appointment) {
  ...
  const redemptions = appointment.creditRedemptions || [];
  if (redemptions.length > 0) {
    // Verificar que TODOS los packs est�n pagados
    return redemptions.every(r => r.creditPack?.paid === true);
  }
  
  // Si tiene priceCents sin redemptions, consideramos pendiente
  // (una sesi�n suelta sin pack asociado est� pendiente hasta que se pague)
  return false;
}
...
router.get('/dashboard', async (req, res, next) => {
  try {
    ...
    // 6. Pacientes nuevos en el per�odo
    ...
    // 9. Distribuci�n de citas por d�a de la semana (para gr�fico)
    ...
    // 10. Ingresos por semana del mes (para gr�fico)
